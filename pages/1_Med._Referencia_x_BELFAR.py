...
CONFIG_BULAS = {
    "Paciente": {
        "secoes": [
            ...
            {"nome": "ONDE, COMO E POR QUANTO TEMPO POSSO GUARDAR ESTE MEDICAMENTO?", "prefixo": "5.", "ignorar_comp": False, "ignorar_orto": False},
...
def mapear_secoes(linhas_estruturadas, tipo_bula):
    ...
    for idx, linha in enumerate(linhas_estruturadas):
        ...
        # --- [CORREÇÃO 3: MAPEAMENTO DE TÍTULO] ---
...
def construir_html_completo(secoes_analisadas, erros_ortograficos, eh_referencia):
    ...
    for diff in secoes_analisadas:
        ...
        # --- [INÍCIO DA CORREÇÃO 1: LAYOUT REFERÊNCIA] ---
        # Usa o título canônico para a Referência, e o encontrado para o Belfar
        if eh_referencia:
            # Garante que a Referência sempre mostre o título canônico esperado
            titulo_display = f"{prefixo} {secao_canonico}".strip()
        else:
            # Belfar mostra o título que encontrou (ou o canônico se não achar)
            titulo_encontrado = diff['titulo_encontrado_belfar'] or secao_canonico
            # Tenta adicionar o prefixo ao título encontrado se ele já não o tiver
...
st.divider()
st.caption("Sistema de Auditoria de Bulas vPerfeito-R3 | Layout Unificado | Enumeração Profissional")
