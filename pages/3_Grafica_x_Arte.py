...
# Aplicativo Streamlit: Auditoria de Bulas (v68 - Detecção Automática de Curvas)
# - Funcionalidade: Verifica se o PDF é imagem (curvas) ou texto.
# - Lógica: Se for curvas -> OCR. Se for texto -> Leitura direta (Sem erros de OCR).
# - Base: v67 (Limpeza Fina + Fluidez).
...
# ----------------- DETECÇÃO DE CURVAS E OCR -----------------

def verificar_se_pdf_em_curvas(doc) -> bool:
    """
    Verifica se o PDF é 'em curvas' (imagem/vetor) ou se tem texto digital.
    Retorna True se precisar de OCR (Curvas).
    Retorna False se for texto nativo.
    """
    texto_acumulado = ""
    # Verifica até 3 páginas para não pesar o processamento
    for i, page in enumerate(doc):
        if i > 2: break 
        texto_acumulado += page.get_text("text")
    
    # Se tiver menos de 50 caracteres identificáveis nas primeiras páginas,
    # assumimos que é um arquivo em curvas (imagem) e ativamos o OCR.
    caracteres_limpos = re.sub(r'\s+', '', texto_acumulado)
    return len(caracteres_limpos) < 50
...
def fluir_texto(texto: str) -> str:
    ...
    Conserta o 'buraco' deixado pela remoção do lixo.
    """
    linhas = texto.split('\n')
    novo_texto = []
    buffer = ""
...
def extrair_texto_inteligente(arquivo, tipo_arquivo):
    ...
    try:
        ...
        texto = ""
        metodo_usado = "" # Para debug/log

        if tipo_arquivo == 'pdf':
            with fitz.open(stream=io.BytesIO(arquivo_bytes), filetype="pdf") as doc:
                # 1. VERIFICAÇÃO INTELIGENTE DE CURVAS
                em_curvas = verificar_se_pdf_em_curvas(doc)
                
                if em_curvas:
                    # Se for curvas (imagem), força o OCR
                    metodo_usado = "OCR (Curvas detectadas)"
                    texto = executar_ocr(arquivo_bytes)
                else:
                    # Se for texto nativo, lê direto (rápido e preciso)
                    metodo_usado = "Leitura Direta (Texto nativo)"
                    for page in doc:
        ...
        if texto:
            ...
            # 2. Corrige OCR (somente se necessário, mas mal não faz rodar sempre para limpar sujeira)
...
st.markdown("Sistema com validação RÍGIDA + Detecção Automática de Arquivos em Curvas.")
st.divider()
...
if st.button("🔍 Iniciar Auditoria Completa", use_container_width=True, type="primary"):
    if not (pdf_ref and pdf_belfar):
        ...
        with st.spinner("Analisando arquivos (Verificação de Curvas + Extração Híbrida + Limpeza)..."):
            ...
            if erro_ref or erro_belfar:
                ...
                c3.metric("Data Ref", data_ref.group(1).strip() if data_ref else "N/A")
                c4.metric("Data Bel", data_bel.group(1).strip() if data_bel else "N/A")
...
st.divider()
st.caption("Sistema de Auditoria v68 | Detecção Automática de Curvas & Limpeza Fina")
