...
#
# Versão v86 - OCR Inteligente Baseado em Conteúdo
# - CORREÇÃO CRÍTICA: Não verifica apenas se "tem texto", mas se tem "Texto de Bula".
# - LÓGICA: Tenta ler nativo. Se não achar frases como "PARA QUE ESTE MEDICAMENTO", força OCR.
# - UI: Nomes alterados para "Arquivo ANVISA" e "PDF da Gráfica".
...
def limpar_lixo_grafico(texto):
    padroes_lixo = [
        ...
        r'página \d+\s*de\s*\d+', r'^\s*\d+\s*$', r'Tipologia', r'Dimensão', 
        r'Dimensões', r'Formato', r'Times New Roman', r'Myriad Pro', 
        r'Arial', r'Helvética', r'Cores?:', r'Preto', r'Black', r'Cyan', 
        r'Magenta', r'Yellow', r'Pantone', r'^\s*\d+[,.]?\d*\s*mm\s*$', 
        r'\b\d{2,4}\s*x\s*\d{2,4}\s*mm\b', r'^\s*BELFAR\s*$', r'^\s*PHARMA\s*$',
        r'CNPJ:?', r'SAC:?', r'Farm\. Resp\.?:?', r'CRF-?MG', r'Cód\.?:?', 
        r'Ref\.?:?', r'Laetus', r'Pharmacode', r'.*AZOLINA:\s*Tim.*', 
        r'.*NAFAZOLINA:\s*Times.*', r'\b\d{6,}\s*-\s*\d{2}/\d{2}\b', 
        r'^\s*[\w_]*BUL\d+V\d+[\w_]*\s*$', r'.*New\s*Roman.*', r'.*r?po\s*10.*',
...
def verifica_qualidade_texto(texto):
    """
    Retorna True se o texto parecer ser uma bula válida.
    Retorna False se parecer lixo técnico (indica necessidade de OCR).
    """
    if not texto: return False
    # Normaliza
    t = unicodedata.normalize('NFD', texto).lower()
    # Palavras-chave que DEVEM existir em uma bula de paciente
    keywords = ["para que este medicamento", "como devo usar", "dizeres legais", "quando nao devo usar", "composicao"]
    
    # Conta quantas keywords foram achadas
    hits = sum(1 for k in keywords if k in t)
    
    # Se achou menos de 2 frases chaves, provavelmente é lixo ou texto oculto de gráfica
    return hits >= 2
...
def extrair_texto_hibrido(arquivo, tipo_arquivo, is_marketing_pdf=False):
    ...
    try:
        ...
        if tipo_arquivo == 'pdf':
            # 1. Tenta ler NATIVO (Rápido)
            texto_nativo = ""
            with fitz.open(stream=io.BytesIO(arquivo_bytes), filetype="pdf") as doc:
                for page in doc:
                    # Se for MKT, usa lógica de colunas/blocos para não misturar
                    if is_marketing_pdf:
                        blocks = page.get_text("blocks")
                        blocks.sort(key=lambda b: (b[1], b[0])) # Ordena vertical depois horizontal
                        for b in blocks:
                            if b[6] == 0: texto_nativo += b[4] + "\n"
                    else:
                        texto_nativo += page.get_text() + "\n"
            
            # 2. A VERIFICAÇÃO CRUCIAL
            # O texto nativo é bom? Tem cara de bula?
            if verifica_qualidade_texto(texto_nativo):
                # Sim, o texto nativo é válido. Usa ele.
                texto_completo = texto_nativo
                metodo = "Nativo (Validado)"
            else:
                # Não, o texto nativo é lixo ou vazio. FORÇA OCR.
                texto_completo = executar_ocr(arquivo_bytes)
                metodo = "OCR (Forçado por Conteúdo)"
        ...
        if texto_completo:
            ...
            print(f"Arquivo: {getattr(arquivo, 'name', '?')} | Método Usado: {metodo}")
            return texto_completo.strip(), None

    except Exception as e:
        return "", f"Erro: {e}"

# ----------------- RECONSTRUÇÃO E ANÁLISE (Mantido igual) -----------------
...
st.markdown("Sistema de conferência Híbrida com OCR automático se a leitura falhar.")
st.divider()
...
if st.button("🔍 Iniciar Auditoria", type="primary", use_container_width=True):
    if not (pdf_ref and pdf_belfar):
        ...
        with st.spinner("Analisando arquivos (Verificando conteúdo real para decidir OCR)..."):
            # Extração independente com verificação de conteúdo
...
st.divider()
st.caption("Sistema v86 | OCR Ativado por Conteúdo Inválido")
